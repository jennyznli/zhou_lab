...
betas = readRDS(file = "~/Documents/HPC_share/thyroid/20240320_thyroid136_betas_condensed.rds") #937690
...
# SPLITTING TRAINING DATA _______________________________________________________________

set.seed(1) # 70/30 split
ss$Train1 <- sample.split(ss$IDAT, SplitRatio = 0.7)

train  <- subset(ss, ss$Train1 == TRUE)
test <- subset(ss, ss$Train1 == FALSE)

# how many in training group for high and low invasive
print(sum((ss$Invasiveness == "High") & (ss$Train1 == TRUE))) # 37/63
print(sum((ss$Invasiveness == "Low") & (ss$Train1 == TRUE))) # 26/63

train_labels <- as.factor(train$Invasiveness)
test_labels <- as.factor(test$Invasiveness)

## SPLIT BETAS
mtx = t(mtx)
train_betas = mtx[, colnames(mtx) %in% train$IDAT]
test_betas = mtx[, colnames(mtx) %in% test$IDAT]
train_betas = t(train_betas)
test_betas = t(test_betas)

# TRAINING MODEL _______________________________________________________________

model1 <- randomForest(train_betas, train_labels, ntree = 500)

# OOB estimate of  error rate: 12.7%
# Confusion matrix:
#     High-invasive Low-invasive class.error
# High-invasive            35            2  0.05405405
# Low-invasive              6           20  0.23076923
saveRDS(model1, "~/Documents/HPC_share/thyroid/20240321_3k_7030_thyroid90_classifier.rds")

test_predictions <- predict(model1, test_betas)
test_accuracy <- sum(test_predictions == test_labels) / length(test_labels)
print(paste("Test Accuracy:", test_accuracy))
# "Test Accuracy: 0.851851851851852"
