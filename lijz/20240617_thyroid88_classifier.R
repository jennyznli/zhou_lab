x<-c("sesame", "RSNNS", "readxl", "readr", "dplyr", "randomForest", "impute", "tidyverse", "caret", "e1071", "pROC")
...
ss = ss %>% dplyr::filter(INCLUDE_IN_ANALYSIS == "1") #88 29
...
##### DIFF METH CPGS __________________________________________________
# df1 = readRDS("~/Documents/HPC_share/thyroid/20240603_thyroid88_dm_invasiveness.rds")
# 
# # pos probes 1199
# diffMeth = (df1$Probe_ID[df1$Est_InvasivenessHigh > 0.2])
# # neg probes 12652
# diffMeth = (df1$Probe_ID[df1$Est_InvasivenessHigh < -0.2])
# # 1k 
# diffMeth = c((df1$Probe_ID[df1$Est_InvasivenessHigh > 0.38]), (df1$Probe_ID[df1$Est_InvasivenessHigh < -0.38]))
# # 2k 
# diffMeth = c((df1$Probe_ID[df1$Est_InvasivenessHigh > 0.34]), (df1$Probe_ID[df1$Est_InvasivenessHigh < -0.34]))
# # 14k 
# diffMeth = c((df1$Probe_ID[df1$Est_InvasivenessHigh > 0.2]), (df1$Probe_ID[df1$Est_InvasivenessHigh < -0.2]))
# 
# dim(betas_knn[rownames(betas_knn) %in% diffMeth, ])
# mtx = t(betas_knn)
# mtx = mtx[rownames(mtx) %in% ss$IDAT, colnames(mtx) %in% diffMeth]
# sum((diffMeth) %in% colnames(mtx)) #check 

##### MOST VARIABLE CPGS____________________________________________________________
...
# }
# mtx = bSubMostVariable(betas, 2000)
# mtx = t(mtx)
# mtx = mtx[rownames(mtx) %in% ss$IDAT,]
...
# svm_model_func <- function(data) {
#     set.seed(1234)
#     svm(invasiveness ~ ., data = data, probability = TRUE)
# }
# 
# mlp_model_func <- function(data) {
#     set.seed(1234)
#     nnet(invasiveness ~ ., data = data, size = 2, decay = 5e-4, linout = FALSE, trace = FALSE, maxit = 1000, MaxNWts=6000)
# }

# Perform cross-validation for each model
# models <- list(randomForest = rf_model_func, SVM = svm_model_func, MLP = mlp_model_func)
# models <- list(randomForest = rf_model_func, SVM = svm_model_func)
